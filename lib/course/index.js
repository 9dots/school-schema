...
var task = Schema().prop('displayName', displayName).prop('url', url).prop('type', activityType).prop('id', Schema('string').faker('random.uuid')).prop('index', Schema('number').multiple(1.0).faker('random.number')).required(['displayName', 'url', 'type', 'index']);
...
var updateTask = Schema().prop('course', firebaseRef).prop('draft', firebaseRef).prop('lesson', uuid).prop('task', uuid).prop('displayName', displayName).prop('type', activityType).required(['task', 'course', 'lesson', 'draft']).others(false, 'invalid_keys');
